...
def steady_state_starts(ts, cpts):
    ...
    lower_bound = np.array(last_segment).mean() - max(np.array(last_segment).var(), 0.001)
    upper_bound = np.array(last_segment).mean() + max(np.array(last_segment).var(), 0.001)
    ...
    while cpts[i] > 0:
        ...
        if not (np.array(current_segment).mean() + np.array(current_segment).var() >= lower_bound and np.array(current_segment).mean() - np.array(current_segment).var() <= upper_bound):
            return cpt
...
def classify_fork(ts, cpts):
    ...
    if steady_state_starts_at <= 1499:
