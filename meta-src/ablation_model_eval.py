...
from evaluate import get_avg_rank, pairwise_wilcoxon, friedman
from visualization import stacked_barchart, error_distribution_with_wilcoxon

from cfg import CONFIG_PATH, ABLATION_MODELS, ABLATION_MODELS_EVAL, ABLATION_MODELS_EVAL_TUNING
...
if __name__ == "__main__":
    ...
    parser.add_argument('--tuning', action='store_true', help="Evaluate tuned models")
    ...
    eval_path = ABLATION_MODELS_EVAL_TUNING if args.tuning else ABLATION_MODELS_EVAL
    ...
    for task in config["tasks"]:
        ...
        for dataset in task['datasets']:
            ...
            if args.tuning:
                meta_result_folder = ABLATION_MODELS.parent / (ABLATION_MODELS.name + "_tuned")
            else:
                meta_result_folder = ABLATION_MODELS

            for meta_result_path in meta_result_folder.glob(f"*{dataset['dataset_id']}---*.csv"):

                output_name = meta_result_path.stem.split("#")[-1]
                ...
                ranks_df.describe().to_csv(eval_path / f"describe_rank#{output_name}.csv")
                ...
                stacked_barchart(rank_percentages, out_path = eval_path / f"rank_count_{output_name}.png")
                ...
                reshaped_metrics.to_csv(eval_path / f"reshaped_metrics#{output_name}.csv", index=False)
                ...
                wilcoxon_results.to_csv(eval_path / f"wilcoxon_posthoc#{output_name}.csv")
                ...
                error_distribution_with_wilcoxon(
                    ...
                    out_path = eval_path / f"combined_plot#{output_name}.png"
                )